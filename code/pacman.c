...
//Some global variables to store the information for output file
...
void ExitProgram(const char *message) {
	...
	int exp_per_sec = (int)total_expanded_nodes/time_spent;
	if((fp = fopen("output.txt","w")) != NULL){
		fprintf(fp,"MaxDepth = %d\nTotalGenerated = %d\nTotalExpanded=%d\nTime = %f seconds\nExpanded/Second = %d\nScore:%d\n",total_max_depth,total_generated_nodes,total_expanded_nodes, time_spent,exp_per_sec, Points);
	}
